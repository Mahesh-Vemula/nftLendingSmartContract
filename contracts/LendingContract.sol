...
contract LendingContract {
	...
	**/
	function lenderApproveLoanRequest() public payable returns (string memory) {
		uint256 nftValue = NFT(loanRequestInfo.nftTokenAddress).getNFTValue(loanRequestInfo.tokenId);
		require(!(NFT(loanRequestInfo.nftTokenAddress).isUnderCollateral(loanRequestInfo.tokenId)), "Provide NFT is UnderCollateral status");
		address nftOwner = NFT(loanRequestInfo.nftTokenAddress).ownerOf(loanRequestInfo.tokenId);
	...
	**/
	function borrowerTakeDisbursement() public {
		...
		require((NFT(loanRequestInfo.nftTokenAddress).getApproved(loanRequestInfo.tokenId)) == address(this), "Contract is not added for NFT approval!");
		loanApplicationStatus = LoanStatus.DISBURSED;
		loadDisburedDate = block.timestamp;
		NFT(loanRequestInfo.nftTokenAddress).updateCollateralStatus(loanRequestInfo.tokenId, true);
	...
	**/
	function payLoanDue() public payable{
		...
		NFT(loanRequestInfo.nftTokenAddress).updateCollateralStatus(loanRequestInfo.tokenId, false);
		//NFT(loanRequestInfo.nftTokenAddress).approve(address(0), loanRequestInfo.tokenId);
		NFT(loanRequestInfo.nftTokenAddress).safeTransferFrom(borrower, borrower, loanRequestInfo.tokenId);
	...
	**/
	function initiateLiquidation() public payable{
		...
		NFT(loanRequestInfo.nftTokenAddress).safeTransferFrom(borrower, msg.sender, loanRequestInfo.tokenId);
